...
session_start();
	...
	if(isset($_GET['kelolaproyek'])){
		$id= $_GET['kelolaproyek'];
	...
	if(isset($_GET['kelolaperjalananDinas'])){
			$id= $_GET['kelolaperjalananDinas'];

			if(!empty($id)){
				mysqli_query($koneksi,"DELETE FROM before_dinas WHERE id_dinas='$id'");

				header("location:../../kelola_perjalananDinas.php");
			}	
	}
